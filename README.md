...
- **Settings:** switch between Modern and Classic layouts, choose from five Modern color palettes, adjust Modern opacity, select from eight built-in WoW font styles, and scale the window from 65% to 110%. An 80–85% scale is recommended for 1080p screens. Classic keeps its old-WoW-inspired raised title plaque, portrait medallion, rounded ornamental borders, beveled red controls, and parchment pages. All choices are saved account-wide.
...
Pushing a tag such as `v2.0.1` runs the GitHub Actions release workflow. It packages the addon with `RevathsMailbox` as the ZIP's top-level folder, uploads the build as a workflow artifact, and attaches the same ZIP to a generated GitHub Release.
